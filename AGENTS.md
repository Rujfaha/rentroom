...
This file defines the rules that every AI coding agent must follow when working on this project.

The goal is to keep the codebase clean, safe, maintainable, and easy for another agent or developer to continue.

---

## UI Design Rules

- Do not use unnecessary emoji in any UI element.
- Emoji must not be used in labels, buttons, badges, options, menus, tabs, cards, or navigation.
- If an icon is needed, always use an inline SVG icon or an existing SVG-based icon component.
- Emoji may only appear in toast messages, notes, comments, or text that was directly entered by the user.
- Example:
  - Bad: `🚶 Walk-in`
  - Good: `<svg>...</svg> Walk-in`
- Always inspect the current UI and follow the existing design direction.
- Do not introduce a new visual style that conflicts with the current system.
- Use a mobile-first approach.
- Every page and component must support responsive layouts across mobile, tablet, and desktop.
- Keep spacing, typography, border radius, shadows, and colors consistent with the existing design system.
- Do not make UI changes that look visually disconnected from the current product.
...
Use this structure unless the existing project already has a different established pattern:

```txt
components/ = UI components
hooks/ = React hooks
lib/ = service, client, database, and integration utilities
...
Before fixing lint errors, TypeScript errors, build errors, or code quality issues, read and follow:

```md
./skill.md
```

The agent must follow the coding and lint-solving guidelines in `skill.md`.

If `skill.md` conflicts with this file, follow the more specific instruction. If the conflict is unclear, explain the conflict before editing.
...
If the project uses another package manager, use the correct equivalent command.
...
The main goal is to make the project safe to continue across multiple agents without losing context, duplicating work, breaking the UI, or damaging existing working code.
