...
export function buildGroundedResponsePrompt(input: Omit<GenerateLineResponseInput, "provider">): string {
  ...
  return [
    ...
    "- If responsePlan.canIncludeBookingUrl is true, you MUST present the EXACT URL from responsePlan.bookingUrl naturally after recommending a room or promotion. Briefly summarize the details and ask if they are interested in proceeding.",
    "- When all booking details are known and the customer asks for a summary, present a complete summary with calculated total price, then provide the EXACT URL from responsePlan.bookingUrl.",
