import { applyStayAvailabilityToContext, getHotelAIContext } from "./hotel-context";
...
export async function generateHospiqReply(input: GenerateHospiqReplyInput): Promise<GenerateHospiqReplyResult> {
  ...
  const normalizedExtraction = normalizeExtractionRoomType(extraction, baseContext.roomtypes);
  const intent = normalizedExtraction.primaryIntent;
  const mergedMemory = mergeBookingLeadFromEntities(baseContext.memory, normalizedExtraction);
  const aiProvider = getAiProvider();
  const relevantFaqs = await retrieveFaqsWithSemanticFallback({
    hotelId: input.hotelId,
    faqs: baseContext.faqs,
    message: input.message,
    language: normalizedExtraction.language,
  ...
  const context = await applyStayAvailabilityToContext(contextWithMemory);
  const policy = resolveAiPolicy(context, intent);
  const handoffRequired = policy.shouldHandoff || normalizedExtraction.handoff?.required === true;
  const prompt = buildStarterPromptPayload(context, input.message, intent, {
    canOfferBookingLink: policy.canOfferBookingLink,
  ...
  return {
    ...
    prompt,
    handoffRequired,
    handoffReason: normalizedExtraction.handoff?.reason ?? null,
    handoffPriority: normalizedExtraction.handoff?.priority ?? null,
    memoryUpdate: draft.memoryUpdate,
    language: normalizedExtraction.language,
    entities: normalizedExtraction.entities,
  };
}
