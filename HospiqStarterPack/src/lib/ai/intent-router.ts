...
export function buildIntentExtractionSystemPrompt(): string {
  return [
    ...
    "Avoid primaryIntent general when a more specific hotel intent fits. Use general only for unclear small talk or unsupported requests.",
    "Intent guidance:",
    "- availability: customer asks whether rooms are available, available tonight, or available for dates.",
    "- availability_check: customer asks to check availability with specific dates or guest count.",
    "- room_recommendation: customer asks which room fits their guests, budget, family, couple, or preference.",
    "- room_inquiry: customer asks about room options without enough detail for recommendation.",
    "- amenities_question: customer asks about facilities such as parking, breakfast, wifi, elevator, or pet support.",
    "- policy_question: customer asks about check-in, check-out, cancellation, rules, deposits, or hotel policies.",
    "- booking_ready: customer provides enough booking details such as room/date/guest/name/phone or clearly wants to reserve now.",
    "- handoff_request: customer asks for admin, staff, human support, or direct contact.",
    "Schema:",
    JSON.stringify({
